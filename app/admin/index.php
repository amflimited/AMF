...
$c = one("SELECT (SELECT COUNT(*) FROM ref_states) states,(SELECT COUNT(*) FROM sources) sources,(SELECT COUNT(*) FROM records) records,(SELECT COUNT(*) FROM rejected_records) rejected,(SELECT COUNT(*) FROM source_candidates) candidates,(SELECT COUNT(*) FROM source_acquisition_evaluations) evaluations,(SELECT COUNT(*) FROM source_sample_runs) samples");
echo '<section class="card"><div class="kpi">';
foreach (['states'=>'States','candidates'=>'Candidates','evaluations'=>'Auto Evals','samples'=>'Samples','sources'=>'Runtime Sources','records'=>'Accepted','rejected'=>'Rejected'] as $k=>$label) echo '<div class="metric"><b>'.h($c[$k] ?? 0).'</b><span>'.h($label).'</span></div>';
echo '</div></section>';

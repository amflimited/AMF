...
if (!preg_match('/^[A-Z]{2}$/', $state) || $url === '') {
    http_response_code(400);
    exit('Missing state or URL.');
}
...
exec_sql("INSERT INTO source_candidates (
    ...
    $state, $stateName, $url, trim($_POST['source_owner'] ?? ''), $type,
    isset($_POST['is_official_government']) ? 1 : 0,
    isset($_POST['has_bulk_download']) ? 1 : 0,
    isset($_POST['has_search_form']) ? 1 : 0,
    isset($_POST['requires_javascript']) ? 1 : 0,
    isset($_POST['requires_captcha']) ? 1 : 0,
    isset($_POST['requires_login']) ? 1 : 0,
    isset($_POST['namecheap_compatible']) ? 1 : 0,
    $parser,
    $status,
    trim($_POST['notes'] ?? '')
]);
